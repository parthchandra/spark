...
case class ResolveProcedures(
    catalogManager: CatalogManager) extends Rule[LogicalPlan] with LookupCatalog {
  ...
  private def validateParams(params: Seq[ProcedureParameter]): Unit = {
    ...
    params.sliding(2).foreach {
      case Seq(previous, current) if !previous.required && current.required =>
        throw new AnalysisException(
          s"Optional params must be after required ones but $current is after $previous")
      case _ =>
    }
  }
