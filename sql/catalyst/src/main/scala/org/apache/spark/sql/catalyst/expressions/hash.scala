 ...
 *                        to `result`.
 *  - map:                If `spark.sql.legacy.allowHashOnMapType` is true, the `result` starts
 *                        with seed, then use `result` as seed, recursively calculate hash value
 *                        for each key-value, and assign the key-value hash value to `result`
