        ...
        try {
          ...
          case ex: InvocationTargetException if ex.getCause.isInstanceOf[MetaException] =>
            throw new RuntimeException("Caught Hive MetaException attempting to get partition " +
              ...
              s"${SQLConf.HIVE_METASTORE_PARTITION_PRUNING_FALLBACK_ON_EXCEPTION.key} to true to " +
              s"work around this problem, however this will result in degraded performance. " +
              "Please report a bug: https://issues.apache.org/jira/browse/SPARK", ex)
        }
      }
