...
private[hive] object IsolatedClientLoader extends Logging {
  ...
  def hiveVersion(version: String): HiveVersion = version match {
    ...
    case "2.3" | "2.3.0" | "2.3.1" | "2.3.2" | "2.3.3" | "2.3.4" | "2.3.5" | "2.3.6" |
         "2.3.7" => hive.v2_3
    case "2.3.8.5-apple" | "2.3.9" => hive.v2_3_8
  ...
  def supportsHadoopShadedClient(hadoopVersion: String): Boolean = {
    VersionUtils.majorMinorPatchVersion(hadoopVersion).exists {
      ...
      case (3, 3, _) if hadoopVersion.contains("apple") => true
      case _ => false
    }
  }
  ...
  private def downloadVersion(
    ...
    val hiveArtifacts = if (!Seq("2.3.8.5-apple", "2.3.9").contains(version.fullVersion))  {
      version.extraDeps ++
