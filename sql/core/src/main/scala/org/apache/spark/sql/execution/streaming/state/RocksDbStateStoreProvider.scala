...
private[sql] class RocksDbStateStoreProvider extends StateStoreProvider with Logging {
  ...
  private def getPath(subFolderName: String, version: Option[String] = None): String = {
    val relateiveCheckpointPath = if (stateStoreId.storeName == StateStoreId.DEFAULT_STORE_NAME) {
        s"${stateStoreId.operatorId}/${stateStoreId.partitionId}"
      } else {
        s"${stateStoreId.operatorId}/${stateStoreId.partitionId}/${stateStoreId.storeName}"
      }
    ...
    val dirPath = new Path(
      ...
      new Path(
        subFolderName,
        relateiveCheckpointPath + "_" + relateiveCheckpointPath.hashCode()))
