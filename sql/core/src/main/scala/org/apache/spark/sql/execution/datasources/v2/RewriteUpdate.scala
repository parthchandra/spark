...
object RewriteUpdate
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case UpdateTable(r: DataSourceV2Relation, assignments, updateCond) if isIcebergRelation(r) =>
      val writeInfo = newWriteInfo(r.schema)
      val mergeBuilder = r.table.asMergeable.newMergeBuilder("update", writeInfo)

      val cond = updateCond.getOrElse(Literal.TrueLiteral)
  ...
  private def buildUpdateProjection(
    ...
    val updatedExprs = assignedExprs.zip(relation.output).map { case (assignedExpr, attr) =>
      // use semanticEquals to avoid unneeded if expressions as we run after operator optimization
