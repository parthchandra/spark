...
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, DynamicFileFilter, DynamicFileFilterWithCardinalityCheck, Filter, LogicalPlan, Project, Repartition}
import org.apache.spark.sql.catalyst.util.SetAccumulator
import org.apache.spark.sql.connector.catalog.Table
import org.apache.spark.sql.connector.distributions.OrderedDistribution
import org.apache.spark.sql.connector.read.{ScanBuilder, SupportsFileFilter}
...
trait RewriteRowLevelOperationHelper extends PredicateHelper with SQLConfHelper with Logging {

  import DataSourceV2Implicits._
  import IcebergImplicits._
  import RewriteRowLevelOperationHelper._

  protected def resolver: Resolver = conf.resolver
  ...
  private def buildFileFilterPlan(
    ...
    val accumulatorExpr = Alias(
      AccumulateFiles(filesAccumulator, fileAttr),
    ...
    val aggSumCol = Alias(
      AggregateExpression(Sum(affectedFilesAttr), Complete, isDistinct = false),
