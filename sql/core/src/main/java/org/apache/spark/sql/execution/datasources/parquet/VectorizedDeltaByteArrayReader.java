 ...
 * interface.
 */
public class VectorizedDeltaByteArrayReader extends ValuesReader
  ...
  private final VectorizedDeltaLengthByteArrayReader suffixReader =
      new VectorizedDeltaLengthByteArrayReader();
  ...
  public void initFromPage(int valueCount, ByteBufferInputStream in) throws IOException {
    ...
    prefixLengthReader.readIntegers(valueCount, prefixLengthVector, 0);
    suffixReader.initFromPage(valueCount, in);
    //use off heap vector if off heap is enabled?
  ...
  public void readValues(int total, WritableColumnVector c, int rowId,
    ...
    suffixReader.readBinary(total, suffixVector, rowId);

    for (int i = 0; i < total; i++) {
      int prefixLength = prefixLengthVector.getInt(rowId + i);
      byte[] suffix = suffixVector.getBinary(rowId + i);
