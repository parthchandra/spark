...
function exit_with_usage {
  echo "builder.sh <scala-version 2.11|2.12>"
  echo ""
  echo ""
  exit 1
}
...
execute_command "$MVN" com.apple.cie.rio:rio-maven-plugin:create-marker "$@"
...
execute_command "$MVN" clean deploy -DaltDeploymentRepository="${REPO_URL}" $ADDITIONAL_MAVEN_PARAMS "$@"
