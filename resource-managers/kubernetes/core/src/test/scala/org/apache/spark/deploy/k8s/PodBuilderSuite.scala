...
abstract class PodBuilderSuite extends SparkFunSuite {
  ...
  test("configure a custom test step") {
    ...
    val pod = buildPod(sparkConf, client)
    verifyPod(pod)
    assert(pod.containers.flatMap(_.getVolumeMounts.asScala).exists(_.getName == "so_long"))
    assert(pod.containers.flatMap(_.getVolumeMounts.asScala).exists(_.getName == "so_long_two"))
  }
  ...
  override def configurePod(pod: SparkPod): SparkPod = {
    ...
    val podWithLocalDirVolumes = new PodBuilder(pod.pod)
      .editSpec()
        ...
        .endSpec()
      .build()
    val containerWithLocalDirVolumeMounts = pod.containers.map {
      c => new ContainerBuilder(c)
        .addNewEnv()
